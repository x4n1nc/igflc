...
const question = [
  ...
  },
  {
    type:'input',
    name:'hastag',
    message:'Insert Hashtag (Without #)Contoh terkenal #love #instagram #mobilelegend #moba #dangdut',
    validate: function(value){
      if(!value) return 'Isi dulu gan';
      return true;
    }
  },
  {
    type:'input',
    name:'text',
    message:'Masukan komentar 1 (Gunakan Pemisah | bila lebih dari 1)',
    validate: function(value){
      if(!value) return 'Isi dulu gan';
      return true;
    }
  },
  {
    type:'input',
    name:'sleep',
    message:'waktu ? 600000*bisa seharian/ tergantung komentarnya juga(In MiliSeconds)',
    validate: function(value){
...
const doMain = async (account, hastag, sleep, text) => {
  ...
  console.log(chalk`{green [!] Mencoba lagi FLC All Account In Hashtag : ${hastag}\n}`);
  try {
    var cursor;
    var count = 0;
    console.log(chalk`[=== Menjalankan flc5/${sleep} MiliSeconds ===]`)
    do {
      ...
      for (media of media) {
        ...
        console.log(chalk`[{bold.yellow <=== Menjalankan FLC ${sleep} MiliSeconds ===>}]`)
        await delay(sleep);
      }
...
console.log(chalk`recode
      CCOCOT.CO | BC0DE.NET | NAONLAH.NET | WingkoColi | x4ninc
`)
